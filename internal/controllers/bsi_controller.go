...
func (bc *BSIController) AddNewBSI(c *gin.Context) {
	...
	for _, userID := range req.UserIDs {
		...
		newAktivasiAkun := models.AktivasiAkun{
			...
			UserID:      userID,
			AsRole:      models.RoleUserAdmin, // ini admin
			Token:       hashedOTP,
			ExpiredAt:   now.Add(24 * time.Hour),
			GeneratedBy: req.AdminID, // Bisa dengan user yang create asalkan di-pass. Di AddNewBSI params ga ada yang generate. Set system
		}
...
func (bc *BSIController) GetNasabahBSI(c *gin.Context) {
	bankID := c.Param("bank_id")
	if bankID == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Bank ID is required"})
		return
